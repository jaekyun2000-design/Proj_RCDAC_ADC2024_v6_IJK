...
from KJH91_Projects.Project_ADC.Layoutgen_code.H01_CDAC_Driver_v2_YJH_RCHybrid import H01_01_DriverArray
from KJH91_Projects.Project_ADC.Layoutgen_code.H01_CDAC_Driver_v2_YJH_RCHybrid import H01_01_DriverArray_Arranged
...
if __name__ == '__main__':
    ...
    libname = 'Proj_ZZ00_RcdacSar_H02_00_CDACWtDriver_Arranged'
    ## CellName: ex)C01_cap_array_v2_84
    cellname = 'H02_00_CDACWDriver_YJH_RCHybrid'
    ...
    InputParams = dict(
        ...
        _CDAC_ShieldingLayer                =   4,     # GND로 연결되는 Shielding Layer Poly:0, M1:1, M2:2 ...
        _CDAC_BotNodeVtcExtensionMetalLayer =   2,     # Unit cap을 combin 하는 Hrz M3까지 거리 띄우기 위한 Vtc metal
        _CDAC_MetalWidth                    =   50,    # MOM cap의 metal 두께
        _CDAC_MetalLength                   =   1410,  # MOM cap의 Length
        _CDAC_MetalSpacing                  =   50,    # MOM cap의 거리

        # #Unit Cap
        _CDAC_NumOfElement  = 2, #MOM cap 몇개를 묶어서 하나의 Unit cap으로 할것인가

        # # Shielding & Top Connect node
        _CDAC_ConnectLength =   400,    #shielding Layer에서 위쪽방향으로 extension 될때 거리
        _CDAC_ExtendLength  =   700,    #shielding layer extension후에 M7으로 연결되는 Via부분 length

        # # Dummy Cap Option
        _CDAC_DummyCap_TopBottomShort   = None,    # True/False, False: Top plate를 GND로 Short 시킴(Top=Bot=GND), True: Top bot를 같은 node로 묶고 floating시킴
        _CDAC_NumOfDummyCaps            = 7,       # Number of dummy cap(one side)


        # # CommonCentroid With Driving node
        _CDAC_CapArrayWDrivingNodeDistance  = 1500, # DRC Rule, "_CDAC_BotNodeVtcExtensionMetalLayer"의 길이 = CDAC과 Hrz M3(combine)사이 거리
        _CDAC_DriveNodeDistance = 279,              # 279,  # Hrz M3와 Driver 사이 거리
        ...
        _Driver_SizeByBit   =   [8,4,2,1],    #

        # Driver(Inverter) NMOS
        _Driver_NMOS_NumberofGate   = 1,    # number
        _Driver_NMOS_ChannelWidth   = 340,  # number
        _Driver_NMOS_Channellength  = 30,   # number
        _Driver_NMOS_GateSpacing    = None,  # None/number --> Fixed
        _Driver_NMOS_SDWidth        = None,  # None/number --> Fixed
        _Driver_NMOS_XVT            = 'SLVT',  # 'XVT' ex)SLVT LVT RVT HVT
        _Driver_NMOS_PCCrit         = None,  # None/True --> Fixed

            # Source_node_ViaM1M2
        _Driver_NMOS_Source_Via_TF = True,  # True/False --> Fixed

            # Drain_node_ViaM1M2
        _Driver_NMOS_Drain_Via_TF = True,  # True/False --> Fixed

            # POLY dummy setting
        _Driver_NMOS_Dummy = True,              # TF --> Fixed
            # if _PMOSDummy == True
        _Driver_NMOS_Dummy_length = None,       # None/Value --> Fixed
        _Driver_NMOS_Dummy_placement = None,    # None/'Up'/'Dn'/l --> Fixed

        # Driver(Inverter) PMOS
        _Driver_PMOS_NumberofGate = 1,  # number
        _Driver_PMOS_ChannelWidth =900,  # number
        _Driver_PMOS_Channellength=30,  # number
        _Driver_PMOS_GateSpacing=None,  # None/number --> Fixed
        _Driver_PMOS_SDWidth=None,  # None/number --> Fixed
        _Driver_PMOS_XVT='SLVT',  # 'XVT' ex)SLVT LVT RVT HVT
        _Driver_PMOS_PCCrit=None,  # None/True --> Fixed

            # Source_node_ViaM1M2
        _Driver_PMOS_Source_Via_TF = True,  # True/False --> Fixed

            # Drain_node_ViaM1M2
        _Driver_PMOS_Drain_Via_TF = True,  # True/False --> Fixed

            # POLY dummy setting
        _Driver_PMOS_Dummy=True,            # TF --> Fixed
            # if _PMOSDummy == True
        _Driver_PMOS_Dummy_length=None,     # None/Value --> Fixed
        _Driver_PMOS_Dummy_placement=None,  # None/'Up'/'Dn'/ --> Fixed
    )
