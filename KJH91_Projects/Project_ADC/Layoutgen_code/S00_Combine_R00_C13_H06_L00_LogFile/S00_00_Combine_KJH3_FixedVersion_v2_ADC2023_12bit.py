...
if __name__ == '__main__':
	...
	'Shcematic version: Proj_ADC_B04_ADC_Design0/E10_TopSchematic_A08_Top_12bit_LVS'
	## LibraryName: ex)Proj_ADC_A_my_building_block
	libname = 'Proj_ZZ01_S00_00_R00C13H06L00_v2_ADC2023_12bit'
...
## SAR Logic
	## Clock Tree (Driving SAR Logic)
		## Clock Tree Updward (CLK Source Tree, CLK Samp Tree)
			...
			_LOGIC_CLKBufTreeTop_BufTree1_SizeofEachStage=[1, 1, 1, 1],  # CLKSampBuf: [Buf0, Buf1, Buf2 ...] ##--->check
			_LOGIC_CLKBufTreeTop_BufTree2_SizeofEachStage=[1, 1, 1, 1],  # CLKSrcBuf: [Buf0, Buf1, Buf2 ...] ##--->check
		...
		## Clock Tree Dnward (CLK Source Tree, CLK Samp Tree)
			...
			_LOGIC_CLKBufTreeBot_BufTree1_SizeofEachStage=[1, 1, 1, 1],  # CompOutBuf: [Buf0, Buf1, Buf2 ...] ##--->check
			_LOGIC_CLKBufTreeBot_BufTree2_SizeofEachStage=[1, 1, 1, 1],  # CLKDoutBuf: [Buf0, Buf1, Buf2 ...] ##--->check
	...
	## SetResetDFF
		...
		## Master Nor1
			...
			## PMOS
				...
				_LOGIC_Mst_Nor1_PMOSB_ChannelLength           = 30, ##--->chcheckeck
	...
	## DAC PreDriver
		## Inverter Chain
			...
			## Inverter Chain
				## Inv1 common
			_LOGIC_DACPreDrv_NumberofGate=[[8,32],[8,32],[8,32],[8,32],[8,32],[8,32],[8,32],[8,32],[8,32],[8,32],[8,32],[8,32],],  # Vector  ##--->check
...
## Comparator
	## StrongARM Latch
		# Common
		_COMP_SALatch_ChannelLength=60,  # random.randrange(30, 60, 2),  # 30  ##--->check
		_COMP_SALatch_XVT='HVT',  ##--->check

		# PMOS
		_COMP_SALatch_CLKinputPMOSFinger1=4,  #PMOS Reset Sw1 # random.randint(1, 15),  # 6 ##--->check
		_COMP_SALatch_CLKinputPMOSFinger2=4,  #PMOS Reset Sw2 # random.randint(1, 15),  # 3 ##--->check
		...
		_COMP_SALatch_NMOSFinger=15,  		  #NMOS at Latch # random.randint(1, 15),  # 2 ##--->check
...
## CLOCK AND DATA BUF : Comparator Dout Buf for driving Logic, CLKSamp and CLKScr Buff for Driving Sampler and Comparator
	## Top placed
		...
		_BUF_CLKSamp_NumberofGate = [1,1,1,4,16,32],  # Vector,  ##--->check
		...
		_BUF_CLKScr_NumberofGate = [1,1,1,4],  # Vector, ##--->check
...
## CDAC
	...
	_CDACDrv_ArrangedTF = True, #True= Arranged, False=CommonCent  ##--->check
	## CDAC Drv
		## Unit DRv
			## Common
				# XVT
		...
		_CDACDrv_Decimation_Factor=[4, 2, 1, 1, 1, 1, 1, 1, 1, 1],   ##---> no use
		## Option2: (Arragned Only, _CDACDrv_ArrangedTF = 'True')
		_CDACDrv_DesignatedEachSize = [64, 64, 64, 64, 64, 64, 32, 16, 8, 4, 2, 1], # Vector, designates each driver size(multiples of Unit Drv size) ##--->check
	## CDAC
			## Element CDAC
			_CDAC_LayoutOption=[3,4,5,6],  # Vector, Consecutive Number [3,4,5] ##--->check
			_CDAC_MetalWidth=50,  # Number ##--->check
			_CDAC_MetalLength=5770,  # Number (8bit size: 1414/ 10bit size: 2920/ 12bit size: 4200) ##--->check
...
## Bootstrapped Switch
	...
	# TR1
		_Sampler_Tr1_NumberofGate	            = 4,       # Number ##--->check
		_Sampler_Tr1_ChannelWidth	            = 1000,     # Number ##--->check
		_Sampler_Tr1_ChannelLength	            = 30,       # Number ##--->check
		_Sampler_Tr1_XVT				        = 'HVT',    # 'XVT' ex)SLVT/LVT/RVT/HVT ##--->check
	# TR2
	...
	# TR3
		_Sampler_Tr3_NumberofGate	            = 2,       # Number ##--->check
	...
	# TR10
		_Sampler_Tr10_NumberofGate	            = 2,       # Number ##--->check
	...
	# HDVNCAP(232fx3)
		...
		_Sampler_HDVNCAP_NumFigPair = 62, #number (ref:75) ##--->check
