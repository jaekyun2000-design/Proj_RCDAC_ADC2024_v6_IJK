...
class _Pulldown(StickDiagram_KJH1._StickDiagram_KJH):
	...
	def _CalculateDesignParameter(self,
		...
		## ########## ########## ########## ########## ########## ########## ########## ########## ########## ########## ##########  BND nmos Drain connect Hrz m2
		## Boundary_element Generation
		## Generate Boundary_element: ex)LayerName: METAL1 / DIFF (_ODLayer) / POLY / PIMP (_PPLayer) / NWELL / SLVT LVT RVT HVT / OP(OPpress) / CONT (CA) / PCCRIT
		self._DesignParameter['BND_Nmos_DrainConn_Hrz_M2'] = self._BoundaryElementDeclaration(
		...
		self._DesignParameter['BND_Nmos_DrainConn_Hrz_M2']['_YWidth'] = tmp[0][0][0]['_Ywidth']
		
		## Define Boundary_element _XWidth
		tmp1 = self.get_param_KJH4('SRF_NMOS0', 'BND_Drain_Hrz_M2')
		tmp2 = self.get_param_KJH4('SRF_NMOS{}'.format(_Num_Input - 1), 'BND_Drain_Hrz_M2')
		self._DesignParameter['BND_Nmos_DrainConn_Hrz_M2']['_XWidth'] = abs(tmp1[0][0][0]['_XY_right'][0] - tmp2[0][0][0]['_XY_left'][0])
		
		## Define Boundary_element _XYCoordinates
		self._DesignParameter['BND_Nmos_DrainConn_Hrz_M2']['_XYCoordinates'] = [[0, 0]]
		...
		## Approaching_coord: _XY_type2
		tmp2 = self.get_param_KJH4('BND_Nmos_DrainConn_Hrz_M2')
		approaching_coord = tmp2[0][0]['_XY_down_right']
		## Sref coord
		tmp3 = self.get_param_KJH4('BND_Nmos_DrainConn_Hrz_M2')
		Scoord = tmp3[0][0]['_XY_origin']
		## Cal
		New_Scoord = self.get_Scoord_KJH4(target_coord, approaching_coord, Scoord)
		tmpXY.append(New_Scoord)
		...
		## Define coordinates
		self._DesignParameter['BND_Nmos_DrainConn_Hrz_M2']['_XYCoordinates'] = tmpXY
...
if __name__ == '__main__':
	...
	libname = 'Proj_ZZ01_Q01_00_Pulldown'
	cellname = 'Q01_00_Pulldown_99'
	...
	InputParams = dict(
		...
		# Source_node setting
		# Via setting
		_NMOS_Source_Via_TF              = False,   # True/False --> fixed
		# Via close to POpin
		_NMOS_Source_Via_Close2POpin_TF  = False,    # True/False --> dont care
		# Comb setting: If Via is True
		_NMOS_Source_Comb_TF             = False,     # True/False --> dont care
		# Comb POPinward
		_NMOS_Source_Comb_POpinward_TF   = False,    # True/False --> dont care
		# Comb vertical_length
		_NMOS_Source_Comb_Length         = None,     # None/Number --> dont care
		
		# Drain_node_setting
		# Via setting
		_NMOS_Drain_Via_TF               = True,     # True/False --> fixed
		# Via close to POpin
		_NMOS_Drain_Via_Close2POpin_TF   = True,     # True/False --> fixed
		# Comb setting: If Via is True
		_NMOS_Drain_Comb_TF              = True,     # True/False --> fixed
		# Comb POPinward
		_NMOS_Drain_Comb_POpinward_TF    = True,     # True/False --> fixed
		# Comb vertical_length
		...
		# XVT setting : None(Cent), Up, Dn
		_NMOS_Xvt_Placement              = 'Dn',     # None/'Up'/'Dn'/ --> fixed
