...
from KJH91_Projects.Project_ADC.Layoutgen_code.H01_CDAC_Driver_v2_YJH_RCHybrid_Fixed import H01_00_DriverCell
...
#CDAC Drv
    ...
    _CDACDrv_DesignatedEachSize = [8,4,2] # Vector, designates each driver size(multiples of Unit Drv size)
        ...
        ## CALCULATION START
        DriverArray_Arranged_start_time = time.time()
        print('#########################################')
        print('##    CDAC Driver Calculation_Start    ##')
        print('#########################################')
        ## Error
        # if _NumOfBits != len(_CDACDrv_DesignatedEachSize):
        #     raise NotImplementedError(f"_NumberofBis ! = _CDACDrv_DesignatedEachSize ")
        ...
        for i in range(_NumOfBits):
            ...
            _Caculation_Parameters0['_CDACDrv_NMOS_NumberofGate'] = _CDACDrv_NMOS_NumberofGate * _CDACDrv_DesignatedEachSize[i]
            _Caculation_Parameters0['_CDACDrv_PMOS_NumberofGate'] = _CDACDrv_PMOS_NumberofGate * _CDACDrv_DesignatedEachSize[i]

            self._DesignParameter['SRF_{}C_DriverCell'.format(CapSize)] = self._SrefElementDeclaration(_DesignObj=H01_00_DriverCell._DriverCell(_DesignParameter=None, _Name='{}:SRF_{}C_DriverCell'.format(_Name, CapSize)))[0]
        ...
        _Caculation_Parameters0['_CDACDrv_NMOS_NumberofGate'] = 1
        _Caculation_Parameters0['_CDACDrv_PMOS_NumberofGate'] = 1
        ...
        tmp1 = self.get_param_KJH4('SRF_Dummy_DriverCell', 'SRF_Driver_PMOS', 'BND_{}Layer'.format(_CDACDrv_XVT))
        ...
        ## Calculate
        ## Target_coord: _XY_type1
        tmp1 = self.get_param_KJH4('SRF_Dummy_DriverCell', 'SRF_Driver_NMOS', 'BND_{}Layer'.format(_CDACDrv_XVT))
        ...
        tmp0 = self.get_param_KJH4('SRF_{}C_DriverCell'.format(MaxCapSize), 'SRF_Driver_PMOS', 'BND_{}Layer'.format(_CDACDrv_XVT))
        ...
        ## Boundary_element Generation
        self._DesignParameter['BND_PMOSArray_{}Layer'.format(_CDACDrv_XVT)] = self._BoundaryElementDeclaration(
            _Layer=DesignParameters._LayerMapping[_CDACDrv_XVT][0],
            _Datatype=DesignParameters._LayerMapping[_CDACDrv_XVT][1],
            _XWidth=None,
            _YWidth=None,
            _XYCoordinates=[],
        )
        ...
        tmp0 = self.get_param_KJH4('SRF_{}C_DriverCell'.format(MaxCapSize), 'SRF_Driver_PMOS', 'BND_{}Layer'.format(_CDACDrv_XVT))
        tmp1 = self.get_param_KJH4('SRF_Dummy_DriverCell', 'SRF_Driver_PMOS', 'BND_{}Layer'.format(_CDACDrv_XVT))
        self._DesignParameter['BND_PMOSArray_{}Layer'.format(_CDACDrv_XVT)]['_YWidth'] = max(tmp0[0][0][0][0]['_Ywidth'],tmp1[0][0][0][0]['_Ywidth'])

        ## Define Boundary_element _XWidth
        self._DesignParameter['BND_PMOSArray_{}Layer'.format(_CDACDrv_XVT)]['_XWidth'] = abs(tmp0[0][0][0][0]['_XY_left'][0] - tmp1[0][0][0][0]['_XY_right'][0])

        ## Calculate Sref XYcoord
        tmpXY = []
        ## initialized Sref coordinate
        self._DesignParameter['BND_PMOSArray_{}Layer'.format(_CDACDrv_XVT)]['_XYCoordinates'] = [[0, 0]]
        ...
        ## Approaching_coord: _XY_type2
        tmp1 = self.get_param_KJH4('BND_PMOSArray_{}Layer'.format(_CDACDrv_XVT))
        approaching_coord = tmp1[0][0]['_XY_up_left']
        ...
        tmpXY.append(New_Scoord)
        ## Define coordinates
        self._DesignParameter['BND_PMOSArray_{}Layer'.format(_CDACDrv_XVT)]['_XYCoordinates'] = tmpXY
        ##################################################################################################
        ...
        print('#########################################')
        ## Boundary_element Generation
        self._DesignParameter['BND_NMOSArray_{}Layer'.format(_CDACDrv_XVT)] = self._BoundaryElementDeclaration(
            _Layer=DesignParameters._LayerMapping[_CDACDrv_XVT][0],
            _Datatype=DesignParameters._LayerMapping[_CDACDrv_XVT][1],
            _XWidth=None,
            _YWidth=None,
        ...
        ## Define Boundary_element _YWidth
        tmp0 = self.get_param_KJH4('SRF_{}C_DriverCell'.format(MaxCapSize), 'SRF_Driver_NMOS', 'BND_{}Layer'.format(_CDACDrv_XVT))
        tmp1 = self.get_param_KJH4('SRF_Dummy_DriverCell', 'SRF_Driver_NMOS', 'BND_{}Layer'.format(_CDACDrv_XVT))
        self._DesignParameter['BND_NMOSArray_{}Layer'.format(_CDACDrv_XVT)]['_YWidth'] = max(tmp0[0][0][0][0]['_Ywidth'],tmp1[0][0][0][0]['_Ywidth'])

        ## Define Boundary_element _XWidth
        self._DesignParameter['BND_NMOSArray_{}Layer'.format(_CDACDrv_XVT)]['_XWidth'] = abs(tmp0[0][0][0][0]['_XY_left'][0] - tmp1[0][0][0][0]['_XY_right'][0])

        ## Calculate Sref XYcoord
        tmpXY = []
        ## initialized Sref coordinate
        self._DesignParameter['BND_NMOSArray_{}Layer'.format(_CDACDrv_XVT)]['_XYCoordinates'] = [[0, 0]]
        ...
        ## Approaching_coord: _XY_type2
        tmp1 = self.get_param_KJH4('BND_NMOSArray_{}Layer'.format(_CDACDrv_XVT))
        approaching_coord = tmp1[0][0]['_XY_up_left']
        ...
        tmpXY.append(New_Scoord)
        ## Define coordinates
        self._DesignParameter['BND_NMOSArray_{}Layer'.format(_CDACDrv_XVT)]['_XYCoordinates'] = tmpXY
        ##################################################################################################
        ...
        for i in range(_NumOfBits):
            ...
            self._DesignParameter['BND_{}C_Driver_Input_Hrz_M3'.format(CapSize)]['_XWidth'] = max(abs(tmp[0][0][0]['_XY_left'][0] - tmp[-1][0][0]['_XY_left'][0]) + 244, 244) # 244는 컨택 두 개가 박힌 최소 비아 길이
        ...
        ##################################################################################################
            ...
            self._DesignParameter['SRF_{}C_Driver_InputNode_ViaM2M3'.format(CapSize)] = self._SrefElementDeclaration(_DesignObj=A02_ViaStack_KJH3._ViaStack(_DesignParameter=None, _Name='{}:SRF_{}C_Driver_InputNode_ViaM2M3'.format(_Name, CapSize)))[0]
        ...
        print('##############################')
        DriverArray_Arranged_end_time = time.time()
        self.DriverArray_Arranged_elapsed_time = DriverArray_Arranged_end_time - DriverArray_Arranged_start_time
        ##################################################################################################
...
if __name__ == '__main__':
    ...
    libname = 'Proj_ZZ01_H01_01_DriverArray_Arranged_Fixed'
    ## CellName: ex)C01_cap_array_v2_84
    cellname = 'H01_01_DriverArray_Arr'
...
#CDAC Drv
    ## Unit Drv
        ...
        _CDACDrv_PMOS_ChannelLength = 30, #Number
    ## Option(Arragned Only)
    _CDACDrv_DesignatedEachSize = [8,4,2,3] # Vector, designates each driver size(multiples of Unit Drv size)
